...
int build_stack(char* s, sbstring* str, variables* vars, int* vars_size)
{
    if (!strcmp(s, "REM")) {
        ...
        if (!tok) {
            fprintf(stderr,
                    "sbt:\e[31m syntax error:\e[39m %s: The operand was "
                    "expected\n",
                    s);
            return ERROR_CODE;
        }
        ...
        if (!is_all_uppercase(tok) && !is_count(tok)) {
            fprintf(stderr,
                    "sbt:\e[31m syntax error:\e[39m %s: Invalid variable "
                    "name\n",
                    s);
            return ERROR_CODE;
        }
            ...
            if (!strcmp(tok, "=")) {
                while ((tok = strtok(NULL, " "))) {
                    ...
                    if (!strcmp(tok, "+") || !strcmp(tok, "-")) {
                            ...
                            is_all_uppercase(tok) || is_count(tok)
        ...
        *instructionSize += 1;
        ...
        if ((tok = strtok(NULL, " ")) && !strcmp(tok, "=")) {
            ...
            if ((addr = var_getaddress(vars, name)) == -1) {
                fprintf(stderr,
                        "sbt:\e[31m fatal error:\e[39m: Undeclared variable: "
                        "%s\n",
                        tok);
                return ERROR_CODE;
            }
...
int sbt(const char* filepath, const char* result)
{
    ...
    for (int i = 0; !feof(file); ++i) {
        ...
        if (strs[i].n == 0 && (strcmp(tok, "00") || strcmp(tok, "000"))) {
            fprintf(stderr,
                    "sbt:\e[31m fatal error:\e[39m %s: Invalid instruction "
                    "number\n",
                    tok);
            return ERROR_CODE;
        }
    ...
    if (pos_late_addresses) { //? fix instructions
        for (int i = 0; i < pos_late_addresses; ++i) {
            if (late_addresses[i].n == -1) {
                int a = get_sa_address(
                        si, instructionSize, late_addresses[i].old);
                if (a == -1) {
                    fprintf(stderr,
                            "sbt:\e[31m fatal error:\e[39m: Invalid address: "
                            "%d\n",
